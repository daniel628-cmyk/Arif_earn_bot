    ...
    def update_ad_progress(ad_id: int, user_id: int):
        ...
        try:
            with conn.cursor() as cur:
                ...
                # 4. Award user - FIXED: Using 'earned_balance' instead of 'amount'
                reward = 0.30
                cur.execute("""
                    INSERT INTO balances (user_id, earned_balance)
                    VALUES (%s, %s)
                    ON CONFLICT (user_id) 
                    DO UPDATE SET earned_balance = balances.earned_balance + EXCLUDED.earned_balance
                """, (user_id, reward))
